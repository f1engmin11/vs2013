...
/////////////////////////////////////////////////////////////////////////////
SCREEN_STATES  screenState = CREATE_BUTTONS; // current state of main demo state mashine, 当前页面状态
...
{
    //ckInit();     			// Start tick counter
    GOLInit();      			// Initialize graphics library and crete default style scheme for GOL
    //epInit();     			// Initialize beeper
    //uchInit();    			// Initialize touch screen
    //CCInit(); 				// Setup the RTCC
...
//---------------------------------------------------------------
// 生成一个页面：页面标题、前一页按钮、下一页按钮、时钟静态文本框
...
void CreateMeter() 
{
	...
	MtrCreate(ID_METER1,
			  ...
			  MTR_DRAW|MTR_RING,	   	// draw meter object with ring scale
			  0,				   		// set initial value
...
WORD MsgPicture(WORD objMsg, OBJ_HEADER* pObj)
{
    switch(GetObjID(pObj)){
        ...
        case ID_CHECKBOX1:
            if(objMsg == CB_MSG_CHECKED){ 	// set new scale factor
        ...
        case ID_CHECKBOX2:
            if(objMsg == CB_MSG_CHECKED){ 	// switch on/off animation
...
WORD PanelPotentiometer()
{
    ...
    if(state == POTPNL_STATE_SET){ 		// set data for panel
	...
	if (state == POTPNL_STATE_DRAW) {
	    if(!GOLPanelDrawTsk()){ 		// draw box for graph
	       return 0; 					// drawing is not completed
	    }else{
	    	state = POTPNL_STATE_TEXT;	// change to initial state
		    SetFont((void*)&GOLMediumFont);
			SetColor(BRIGHTBLUE);
//			MoveTo(GetMaxX()-(GetTextWidth(UseR6PotStr ,(void*)&GOLMediumFont)+30),215);
			MoveTo(GetMaxX()-(GetTextWidth("Using pseudo-random samples...",(void*)&GOLMediumFont)+30),215);
	    }
	}
	if (state == POTPNL_STATE_TEXT) {
//		while(!OutText(UseR6PotStr))		// "Using R6 potentiometer"))
...
WORD PanelECG()
{
    ...
    switch(state){
        ...
        case ECG_STATE_HORLINE:
			...
			MoveTo(GetMaxX()-(GetTextWidth(ScanModeStr ,(void*)&GOLMediumFont)+30),215);
            state = ECG_STATE_TEXT; 		// change to text display state

        case ECG_STATE_TEXT:
			while(!OutText(ScanModeStr))		// while(!OutText("Scan mode"))
				return 0;
