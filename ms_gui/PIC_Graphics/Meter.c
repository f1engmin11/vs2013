    ...
    switch(state) {
        ...
        case TEXT_DRAW:	
			...
			// set the start location of the meter title
				...
				MoveTo(((pMtr->right+pMtr->left)>>1)-(temp>>1), pMtr->yCenter+(GetTextHeight(pMtr->pGolScheme->pFont)>>2));
...
mtr_needle_draw_st:
		...
		case VALUE_ERASE:
			...
			SetColor(pMtr->hdr.pGolScheme->Color0);
				...
				Bar(    pMtr->xCenter-(temp>>1),
			 			pMtr->yCenter-GetTextHeight(pMtr->pGolScheme->pFont),
					    pMtr->xCenter+(temp>>1),
					    pMtr->yCenter);
		...
		case VALUE_DRAW:
			...
			temp = GetTextWidth(&strVal[SCALECHARCOUNT-j], pMtr->hdr.pGolScheme->pFont);
				...
				MoveTo(pMtr->xCenter, pMtr->yCenter-GetTextHeight(pMtr->pGolScheme->pFont));
